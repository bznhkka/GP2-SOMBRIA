...
public class BETA_SETTINGS{
    ...
    private void FixedUpdate()
    {
        ...
        if(useStamina){
            isSprinting = Input.GetKey(sprintKey) && !isCrouching && staminaInternal > 0 && (Mathf.Abs(fps_Rigidbody.linearVelocity.x) > 0.01f || Mathf.Abs(fps_Rigidbody.linearVelocity.x) > 0.01f);
            if(isSprinting){
            ...
            }else if((!Input.GetKey(sprintKey)||Mathf.Abs(fps_Rigidbody.linearVelocity.x)< 0.01f || Mathf.Abs(fps_Rigidbody.linearVelocity.x)< 0.01f || isCrouching)&&staminaInternal<staminaLevel){
        ...
        if(IsGrounded || fps_Rigidbody.linearVelocity.y < 0.1) {
        ...
        float yv = fps_Rigidbody.linearVelocity.y;
        ...
        if(playerCanMove)
        {
            fps_Rigidbody.linearVelocity = dMove + (Vector3.up * yv);
